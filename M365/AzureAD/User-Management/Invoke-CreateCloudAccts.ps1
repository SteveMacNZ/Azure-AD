<#
.SYNOPSIS
  Create Cloud Only Accounts in a Microsoft 365 Tenant based on CSV file input, Uses MsolService
.DESCRIPTION
  Create Cloud Only Accounts in a Microsoft 365 Tenant based on CSV file input
.PARAMETER None
  None
.INPUTS
  CSV file with following fields (Use CloudUsers.xlsx template)
  UserPrincipalName Email addresss / UPN of the User
  DisplayName       Full / Display Name (Postfix added at the end if applicatble)
...
.OUTPUTS
  ...
  CSV output of created Users in the script root directory
.NOTES
  Version:        1.2
  Author:         Steve McIntyre
  Creation Date:  18/08/2021
  Purpose/Change: Updates for function changes and logic for inital upload to GitHub 
  Version:        1.1
  Author:         Steve McIntyre
...
.EXAMPLE
  .\Invoke-CreateCloudAccts.ps1
#>

#requires -version 4 -Modules MsolService
#---------------------------------------------------------[Script Parameters]------------------------------------------------------
...
$Script:CSVFile             = $PSScriptRoot + "\" + $Script:Date + "_" + "CreatedCloudAcounts.csv"      # CSV Export location and name
$Script:GUID                = 'c0504d07-97a4-4da3-add4-e31d4c7a9312'                                    # Script GUID
...
$Users = Import-csv $Script:File -Delimiter ","                                                     # Load CSV into array
$tc = $Users.count                                                                                  # Count number of imports to be completed
$lc = 0
...
ForEach ($User in $Users) {
    ...
    Try {
        Get-Now
        Write-Host "$Script:Now [INFORMATION] Processing "$_.UserPrincipalName"" -ForegroundColor Yellow  # Write Status Update to Transaction Log
        $USRexist = [bool](Get-MsolUser -UserPrincipalName $_."UserPrincipalName" -erroraction SilentlyContinue)
        if ($USRexist -eq $False){
            Get-Now
            Write-Host "$Script:Now [INFORMATION] User account "$_.UserPrincipalName" not found attempting to create account"
            New-MsolUser -DisplayName $_.DisplayName -FirstName $_.FirstName -LastName $_.LastName `
            -UserPrincipalName $_.UserPrincipalName -UsageLocation $_.UsageLocation | Export-Csv -Path $Script:CSVFile -Append
        }
        else {
            Get-Now
            Write-Host "$Script:Now [INFORMATION] $_."UserPrincipalName" already exists"
        }
        
    }
    Catch {
        Get-Now
        Write-Host "$Script:Now [ERROR] There was an error creating account for "$_.UserPrincipalName"" -ForegroundColor Red
...
Get-Now                                                                                             # Get Current Date Time
Write-Host "$Script:Now [INFORMATION] Restoration of User accounts has been completed, Please see $Script:CSVFile for account details!"
