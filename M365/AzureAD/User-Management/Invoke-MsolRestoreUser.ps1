<#
.SYNOPSIS
  Restores a deleted users in O365 portal from CSV file and removes the ImmutableID, uses legacy MsolService
.DESCRIPTION
  Restores a previous synced account from deleted users in M365 portal from CSV file and removes the ImmutableID
.PARAMETER None
  None
.INPUTS
...
.EXAMPLE
  .\Invoke-MsolRestoreUser.ps1
  Invokes restore of user accounts using MsolService
#>
...
$Script:ScriptName          = 'Invoke-MsolRestoreUser'                                                  # Script Name used in the Open Dialogue
$Script:LogFile             = $PSScriptRoot + "\" + $Script:Date + "_" + $Script:ScriptName + ".log"    # logfile location and name
$Script:GUID                = 'c55f8c17-b634-4ada-8595-0aa7cf93f2ea'                                    # Script GUID
...
ForEach ($Restore in $Restores) {
  ...
  Try {
    ...
    Set-MsolUserPassword -UserPrincipalName $_.UserPrincipalName -ForceChangePassword $True         # Reset Users password and force change on login
  }
  Catch {
    Get-Now
  ...
  Try {
    ...
    Set-MsolUser -UserPrincipalName $_.UserPrincipalName -ImmutableID "$Null"                       # Removes ImmutableID from User account where they were a hybrid account
  }
  Catch {
    Get-Now
